...
class GithubApi extends React.Component {
  ...
  // User and Repository Dropdown handling
   handleUserRepoOptions = (e) => {
      this.setState( {
       UserRepoOptions : e.target.value
      });
    };
  ...
  // this.searchRepos();
  this.updateInput(e);
   Axios.get(`https://api.github.com/${this.state.UserRepoOptions}/${e.target.value}?clientId=${clientId}&clientSecret=${clientSecret}`)
      .then((response) => {
       this.setState({
  ...
  render() {
           ...
           <div>
            ...
            <div className="card-body">
              ...
              <div className="search-bar">
                       <input size ="35" className="search-bar" type="text" value={this.state.username}
                            onChange={this.searchUser}
                            placeholder="Start typing to search .."  />
                            <select value={this.state.UserRepoOptions} onChange={this.handleUserRepoOptions}>
     ...
     <div>
       {
       this.state.UserRepoOptions === "users" ? <GithubProfileCard searchResult={this.state.searchResult} /> :
       <GithubRepo searchResult={this.state.searchResult} />
       }
     </div>
     }
   </Fragment>
